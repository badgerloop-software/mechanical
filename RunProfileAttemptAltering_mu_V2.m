...
trackLength = 1250; % external subrack length - meters
...
y_CG = convlength(7,'in','m'); %height of center of gravity
...
mu_range = 0.4:0.1:0.8; %range of friction coefficients to check
for mu = mu_range
    ...
    torque=50*gear_ratio;   %torque on the wheel initialize
...
%% Propulsion Phase
    while r==0%propulsion phase loop
        ...
        %command_torque_hyper = 50;
        %force_prop = command_torque_hyper/((wD*0.0254)/2) - rolling_drag;
...
%% Braking Phase 
    ...
    full_brake_initialize_secondary(d) = begin_braking_secondary;
    full_time_braking(d) = t_brake;
...
brake_dist = convvel(max_velocity(1,2),'mph','m/s')^2 / (2*decel)
max_RPM_actual = convvel(max_velocity(1,2),'mph','m/s')/((wD*0.0254)/2) * 60/(2*pi())
%force_prop_wheel = command_torque_hyper/((wD*0.0254)/2)
power_real = 50*max_RPM_actual*2*pi()/60
