...
safetyDistance = convlength(50,'ft','m'); %meters 31 for the 100ft rule, 90 for the max braking delay of 1 second between primary and secondary, and 20 as an additional margin 
...
for mu = mu_range
    ...
    run_torque = 20;%Nm
    force_prop = run_torque/((wD*0.0254)/2) - rolling_drag;
...
force_prop_wheel = run_torque/((wD*0.0254)/2)
...
Proof_RPM = max_RPM_actual*1.22

%% Run profile info
run_data = [run_torque,force_prop_wheel,max_RPM_actual,t_prop_secondary,max_velocity(1,2),command_braking,req_brake_dist,safetyDistance,trackLength]';
